    ...
    <div className="mx-auto w-full max-w-5xl space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-sadu-gold bg-white p-6 shadow-xs space-y-4">
          <div className="flex items-center gap-2 border-b border-sadu-gold/30 pb-3">
            <FileText className="h-5 w-5 text-sadu-brick" />
            <div>
              <h2 className="font-editorial text-lg font-bold text-sadu-charcoal">Overarching Curatorial Brief</h2>
              <p className="text-xs text-sadu-muted">Defines exhibition guidelines for Coordinators and Committee</p>
            </div>
          </div>

          <form onSubmit={handleSaveBrief} className="space-y-3">
            <label htmlFor="curatorial-brief-input" className="block text-xs font-semibold text-sadu-charcoal">
              Exhibition Guidelines & Instructions
              <textarea
                id="curatorial-brief-input"
                rows={7}
                required
                value={briefInput}
                onChange={e => setBriefInput(e.target.value)}
                placeholder="Articulate the overarching curatorial parameters, spatial dialogue, medium expectations, and thematic constraints for all participating international artists..."
                className="mt-1 w-full rounded-md border border-sadu-gold/60 p-3 text-xs leading-relaxed text-sadu-charcoal focus:border-sadu-brick focus:outline-none focus:ring-1 focus:ring-sadu-brick"
              />
            </label>

            <div className="flex items-center justify-between">
              <span className="text-[11px] text-sadu-muted">{briefInput.length} characters</span>
              <button
                type="submit"
                className="inline-flex items-center gap-1.5 rounded-md bg-sadu-brick px-4 py-2 text-xs font-bold text-white hover:bg-sadu-brick-dark cursor-pointer"
              >
                <CheckCircle2 className="h-3.5 w-3.5" />
                <span>Save Curatorial Brief</span>
              </button>
            </div>

            {briefSaved && (
              <div className="rounded-md border border-emerald-300 bg-emerald-50 p-2 text-xs font-semibold text-emerald-800 flex items-center gap-2">
                <CheckCircle2 className="h-4 w-4 text-emerald-600" />
                <span>Guidelines saved and synced with Coordinators.</span>
              </div>
            )}
          </form>
        </div>
