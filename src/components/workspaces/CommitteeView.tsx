    ...
    <div className="space-y-6 animate-in fade-in duration-300 pb-12">
      ...
      {viewMode === 'builder' ? <ArtistNominationBuilder lang={lang} onSuccess={() => setViewMode('dossier')} onCancel={() => setViewMode('dossier')} /> : <div className="w-full space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          <section className="space-y-4 rounded-lg border border-sadu-gold bg-sadu-linen p-6 shadow-xs"><h2 className="text-lg font-editorial font-bold text-sadu-charcoal">{isAr ? 'سجل المراجعة السياقية المحددة' : 'Bounded Contextual Review Ledger'}</h2><p className="mb-4 text-xs text-sadu-muted">{isAr ? 'هذا التقييم استشاري ومقيد بنسخة الملف الحالي، ولا يمثل فتوى أو شهادة توثيق مطلقة.' : 'Advisory finding bound to current version only. Does not constitute a religious ruling or universal certification.'}</p><div className="grid gap-4 rounded-md border border-sadu-gold/50 bg-sadu-paper p-4 text-xs md:grid-cols-5"><div><span className="font-bold text-sadu-ink">{isAr ? 'الملاحظة' : 'Observation'}</span><p className="mt-1">{isAr ? 'إعادة إنتاج بيت المتنبي بخط كوفي.' : 'Reproduction of a classical verse in Kufic script.'}</p></div><div><span className="font-bold text-sadu-ink">{isAr ? 'المصدر/الدليل' : 'Evidence'}</span><p className="mt-1">{isAr ? 'مرجع ديوان أبي فراس، ص 153.' : 'Abu Firas Diwan, p. 153.'}</p></div><div><span className="font-bold text-amber-700">{isAr ? 'المخاطر' : 'Risk'}</span><p className="mt-1">{isAr ? 'قد يغير خطأ النسخ الإيقاع التاريخي.' : 'Script transcription error may alter historical cadence.'}</p></div><div><span className="font-bold text-sadu-brick">{isAr ? 'الإجراء المطلوب' : 'Required Action'}</span><p className="mt-1">{isAr ? 'إحالة النص إلى اختصاصي الخط للتحقق من الوصلات.' : 'Refer to a Calligraphy Specialist for exact ligature verification.'}</p></div><div><span className="font-bold text-sadu-ink">{isAr ? 'جهة الاعتماد' : 'Closing Authority'}</span><p className="mt-1">Dr. Ahmad Al-Qassimi</p></div></div><div className="flex items-center justify-between border-t border-sadu-gold/50 pt-2"><span className="text-[10px] font-mono text-sadu-muted">{sampleScriptReviewed ? 'DEMO-SCRIPT-REVIEW' : (isAr ? 'بانتظار مراجعة تجريبية' : 'Awaiting sample review')}</span><button type="button" onClick={() => setSampleScriptReviewed(!sampleScriptReviewed)} className="rounded bg-sadu-brick px-3 py-1.5 text-xs font-bold text-white">{sampleScriptReviewed ? (isAr ? 'إعادة ضبط' : 'Reset') : (isAr ? 'تسجيل مراجعة' : 'Record Check')}</button></div></section>
