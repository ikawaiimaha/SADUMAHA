...
import { Users, FileText, AlertCircle, MessageSquare, ShieldCheck, BellRing, Lock, Send, UserPlus, Clock } from 'lucide-react';
    ...
    <div className="space-y-8 animate-in fade-in duration-300 pb-12">
      ...
      <section className="rounded-lg border border-sadu-gold bg-sadu-linen p-6 shadow-xs sm:p-8">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
          ...
          <div className="flex shrink-0 flex-col gap-2"><button onClick={() => onNavigateTab('communications')} className="flex cursor-pointer items-center justify-center gap-2 rounded-md bg-sadu-brick px-4 py-2.5 text-xs font-bold text-white transition-colors hover:bg-sadu-brick-dark focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sadu-brick"><MessageSquare className="h-4 w-4" />{isAr ? 'مراسلة الفنانين' : 'Message Artists'}</button><button onClick={() => onNavigateTab('dossiers')} className="flex cursor-pointer items-center justify-center gap-2 rounded-md border border-sadu-gold bg-white px-4 py-2.5 text-xs font-bold text-sadu-charcoal transition-colors hover:bg-sadu-sand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sadu-brick"><UserPlus className="h-4 w-4" />{isAr ? 'ترشيح فنان جديد' : 'Nominate Artist'}</button></div>
        </div>
      ...
      <section className="space-y-4 rounded-lg border border-sadu-gold bg-sadu-linen p-6 shadow-xs">
