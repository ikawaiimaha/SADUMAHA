...
export const CoordinatorView: React.FC<CoordinatorViewProps> = (props) => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      {/* FIXED: [&>div]:h-[120px] forces all KpiCards to be exactly 120px tall! */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 [&>div]:h-[120px]">
        <KpiCard
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {[
              ...
              // FIXED: Added md:h-[96px] to lock the height of these rows
              <div key={idx} className="p-4 bg-sadu-sand rounded-lg border border-sadu-gold transition-colors flex flex-col md:flex-row md:items-center justify-between gap-4 md:h-[96px]">
                <div className="flex items-start gap-3 w-full max-w-xl">
                  ...
                  <div>
                    {/* FIXED: line-clamp and fixed heights on text */}
                    <h3 className="font-bold text-sm text-sadu-charcoal line-clamp-1 h-5">{isAr ? item.titleAr : item.titleEn}</h3>
                    <p className="text-xs text-sadu-muted mt-1 line-clamp-2 h-8 leading-relaxed">{isAr ? item.descAr : item.descEn}</p>
                  </div>
                </div>
        ...
        <div className="space-y-4">
          ...
          <div className="bg-sadu-paper border border-sadu-gold rounded-lg p-4 space-y-4">
            {[
              ...
              // FIXED: Locked height to h-[80px]
              <div key={prog.id} className="border-b border-sadu-gold/30 pb-3 last:border-0 last:pb-0 h-[80px] flex flex-col justify-center">
                <div className="flex justify-between items-start mb-1.5">
                  <h3 className="text-xs font-bold text-sadu-charcoal line-clamp-1">{isAr ? prog.titleAr : prog.titleEn}</h3>
