...
export function ProgrammeRoster({ onProposal }: { onProposal: () => void }) {
  const { roster, rosterProgrammes, createProgramme, simulateRosterReview, setProgrammeId } = useArtistIntake();
  const { currentRole, switchRole } = useWorkspace();
  const isCoordinator = ['SDC_COORDINATOR', 'COORDINATOR'].includes(currentRole);
  ...
  return <>
    <section className="lr-panel"><h2>{t('Create a programme draft from the roster', 'إنشاء مسودة برنامج من سجل الفنانين')}</h2>
      ...
      <p className="lr-small">{t('New drafts use a fictional indoor brief, a 3 × 3 × 2.5 m space and an AED 15,000 sample budget ceiling. These are demonstration parameters.', 'تستخدم المسودات الجديدة موجزاً داخلياً افتراضياً ومساحة ٣ × ٣ × ٢٫٥ م وسقف ميزانية تجريبياً قدره ١٥٬٠٠٠ درهم. هذه معايير للعرض فقط.')}</p>
      <form noValidate onSubmit={event => { event.preventDefault(); const result = createProgramme(titleEn, titleAr, selected); setMessage(result ? 'created' : 'invalid'); if (result) { setTitleEn(''); setTitleAr(''); setSelected([]); } }}>
        ...
        <fieldset className="roster-pool"><legend>{t('Select existing artists', 'اختر فنانين من السجل')}</legend><div className="roster-grid">{roster.map(entry => <article key={entry.id} className={`roster-person ${selected.includes(entry.id) ? 'is-selected' : ''}`}>
          ...
          {entry.status === 'pending-review' && <button type="button" onClick={() => isCoordinator ? simulateRosterReview(entry.id) : switchRole('SDC_COORDINATOR')}>{isCoordinator ? t('Record simulated coordinator review', 'تسجيل مراجعة المنسق التجريبية') : t('Switch to sample coordinator', 'الانتقال إلى المنسق التجريبي')}</button>}
          {entry.reviewedAt && <small><bdi>{entry.reviewedBy} · {entry.reviewedAt}</bdi></small>}
