...
export function LivingRecordWorkspace() {
  ...
  const actor: DemoActor = currentRole === 'DIRECTORATE' || currentRole === 'LEADERSHIP' ? leadershipView
    ...
    : ['SDC_COORDINATOR', 'COORDINATOR'].includes(currentRole) ? 'COORDINATOR' : currentRole === 'FINANCE' ? 'FINANCE' : 'OBSERVER';
  ...
  const go = (target: DemoActor) => {
    ...
    if (!role) return;
    if (target === 'CHAIRMAN' || target === 'DIRECTORATE') setLeadershipView(target);
  ...
  return <div className="living-record" lang={lang} dir={isAr ? 'rtl' : 'ltr'}>
    ...
    <main className="lr-main">
      <div className="lr-context"><span>{t('FICTIONAL CASE · SESSION ONLY', 'حالة افتراضية · لهذه الجلسة فقط')}</span><bdi>{CASE_ID}</bdi><span>{t('Mounir Fatmi — demonstration scenario', 'منير فاطمي — سيناريو توضيحي')}</span></div>
      ...
      <header className={`lr-anchor ${leadership ? '' : 'lr-anchor--work'}`}>
        ...
        <div><p className="lr-eyebrow">{t('SHARJAH DEPARTMENT OF CULTURE', 'دائرة الثقافة في الشارقة')}</p>
          ...
          <p className="lr-subtitle">{actor === 'CHAIRMAN' ? t('Chairman of the Department of Culture · Executive exception brief', 'رئيس دائرة الثقافة · موجز الاستثناءات التنفيذية') : actor === 'DIRECTORATE' ? t('Director of Cultural Affairs · Programme delivery and handover', 'مدير إدارة الشؤون الثقافية · تنفيذ البرامج وتسليم المسؤوليات') : t('One record. A clear next action.', 'سجل واحد وخطوة تالية واضحة.')}</p>
      ...
      {actor === 'CHAIRMAN' ? <div className="lr-brief">
        <section className="lr-panel"><p className="lr-eyebrow">{formatNumber(1, { minimumIntegerDigits: 2 })}</p><h2>{t('Active Commitments', 'الالتزامات الجارية')}</h2>
          <p className="lr-metric" data-testid="readiness"><bdi dir="ltr">{formatNumber(metrics.readyCount)} / {formatNumber(metrics.programmes.length)}</bdi></p>
          <p>{t('Sample programmes operationally ready', 'برامج تجريبية جاهزة تشغيلياً')}</p>
          <span className={`lr-status ${metrics.custodyReady ? 'is-clear' : 'is-waiting'}`}>{metrics.custodyReady ? t('Delivery blocker cleared', 'أُزيل معوق التسليم') : t('One programme awaiting handover', 'برنامج واحد بانتظار التسليم')}</span>
          <details className="lr-basis"><summary>{t('Readiness basis', 'أساس الجاهزية')}</summary><p className="lr-small">{t('Three seeded programmes are ready. This case clears only after condition evidence and handover review. Opening authorization remains separate.', 'ثلاثة برامج تجريبية جاهزة مسبقاً. تكتمل جاهزية هذه الحالة بعد أدلة الحالة ومراجعة التسليم. ويبقى إذن الافتتاح منفصلاً.')}</p></details>
          <button className="lr-link" onClick={() => go('DIRECTORATE')}>{t('Review delivery exception', 'مراجعة استثناء التسليم')}<ArrowRight/></button>
        </section>
        <section className="lr-panel"><p className="lr-eyebrow">{formatNumber(2, { minimumIntegerDigits: 2 })}</p><h2>{t('Institutional Memory', 'الذاكرة المؤسسية')}</h2>
          <p className="lr-metric" data-testid="evidence-health">{formatNumber(metrics.evidencePercent)}<span>{isAr ? '٪' : '%'}</span></p>
          <p>{t('Required case evidence present', 'اكتمال أدلة الحالة المطلوبة')}</p>
          <span className="lr-status">{formatNumber(metrics.evidence.length - metrics.evidenceCount)} {t('missing records', 'سجلات ناقصة')}</span>
          <details className="lr-basis"><summary>{t('Evidence basis', 'أساس اكتمال الأدلة')}</summary><p className="lr-small">{t('Five required records: agreement, plan, artist statement, receipt, and condition report. Completeness is not a compliance certification.', 'خمسة سجلات مطلوبة: الاتفاقية والخطة وبيان الفنان والاستلام وتقرير الحالة. اكتمالها ليس شهادة امتثال.')}</p></details>
          {!state.statementPresent && <p className="lr-issue">{t('Artist statement missing · Coordination', 'بيان الفنان ناقص · التنسيق')}</p>}
          <button className="lr-link" onClick={() => go('COORDINATOR')}>{t('Review evidence deficit', 'مراجعة الأدلة الناقصة')}<ArrowRight/></button>
        </section>
        <section className="lr-panel"><p className="lr-eyebrow">{formatNumber(3, { minimumIntegerDigits: 2 })}</p><h2>{t('Pending Escalations', 'التصعيدات المعلقة')}</h2>
          <p className="lr-metric" data-testid="escalations">{formatNumber(metrics.executiveQueue)}</p><p>{t('Items for executive review', 'بنود للمراجعة التنفيذية')}</p>
          {state.finance === 'escalated' ? <><span className="lr-status is-waiting">{t('Delegation validation required', 'يلزم التحقق من التفويض')}</span><button className="lr-link" onClick={() => setModal('finance')}>{t('Review finance exception', 'مراجعة الاستثناء المالي')}<ArrowRight/></button></> : <p className="lr-small">{t('No executive item is routed until Finance supplies its sample pack and the Directorate records review.', 'لا يُرفع أي بند تنفيذي قبل تقديم المالية ملفها التجريبي وتسجيل مراجعة الإدارة.')}</p>}
          <details className="lr-basis"><summary>{t('Authority boundary', 'حدود الصلاحية')}</summary><p className="lr-small">{t('No value threshold or signatory authority is assumed. This demonstration cannot authorize expenditure or apply a signature.', 'لا يُفترض حد مالي أو تفويض توقيع. لا يجيز هذا النموذج الإنفاق أو تطبيق توقيع.')}</p></details>
        </section>
      </div> : <>
        <section className="lr-panel lr-ledger"><div className="lr-section-title"><h2>{t('Delivery and handover', 'التنفيذ والتسليم')}</h2><span className="lr-status">{t('One shared case', 'حالة مشتركة واحدة')}</span></div>
          ...
          <div className="lr-table-wrap"><table><caption className="sr-only">{t('Asset, status, evidence, and next responsible role', 'الأصل والحالة والأدلة والدور المسؤول التالي')}</caption><thead><tr>{[t('Asset', 'الأصل'), t('Location / status', 'الموقع / الحالة'), t('Blocker / evidence', 'المعوق / الأدلة'), t('Next responsible role', 'الدور المسؤول التالي')].map(label => <th key={label} scope="col">{label}</th>)}</tr></thead>
            <tbody><tr><th scope="row">{t('Mounir Fatmi · Crate 4', 'منير فاطمي · الصندوق ٤')}<bdi>{CASE_ID}</bdi></th><td>{status}</td><td>{state.condition ? conditionLink : t('Condition report not yet recorded', 'لم يُسجل تقرير الحالة بعد')}{state.condition?.outcome === 'issue' && <p className="lr-issue">{t('Specialist review required before handover', 'تلزم مراجعة مختصة قبل التسليم')}</p>}</td><td><strong>{roleLabel(metrics.nextActor)}</strong>{actor === 'DIRECTORATE' && !state.acceptance && <button className="lr-primary" disabled={!canReview} onClick={openReview}>{t('Review Handover', 'مراجعة التسليم')}</button>}{metrics.nextActor && metrics.nextActor !== actor && <button className="lr-link" onClick={() => go(metrics.nextActor!)}>{t('Open responsible workspace', 'فتح مساحة الدور المسؤول')}<ArrowRight/></button>}</td></tr></tbody></table></div>
          {state.acceptance && <p className="lr-confirmed"><LockKeyhole aria-hidden="true"/>{t('Demo handover recorded · read-only', 'سُجل التسليم التجريبي · للقراءة فقط')} — <bdi>DEMO-DIRECTORATE</bdi> · {time(state.acceptance.at)} · <bdi>{state.acceptance.reportId}/v{state.acceptance.reportVersion}</bdi></p>}
        </section>
        ...
        {actor === 'DIRECTORATE' && <section className="lr-panel"><h2>{t('Exceptions requiring an assigned next action', 'استثناءات تتطلب إسناد الإجراء التالي')}</h2><div className="lr-exceptions">
      ...
      <p className="lr-announcement" role="status">{newEvent ? t('Latest record: ', 'آخر سجل: ') + eventLabels[newEvent.kind][isAr ? 1 : 0] : t('Start in the Chairman brief, then follow the delivery exception to Logistics.', 'ابدأ بموجز رئيس الدائرة، ثم تابع استثناء التسليم إلى اللوجستيات.')}</p>
    ...
    <dialog ref={dialogRef} className="lr-dialog" aria-labelledby="lr-dialog-title" onCancel={() => setModal(null)} onClose={() => setModal(null)}>
      <div className="lr-dialog-top"><p className="lr-eyebrow">{t('DEMONSTRATION DOSSIER', 'ملف تجريبي')}</p><button autoFocus onClick={() => setModal(null)} aria-label={t('Close dossier', 'إغلاق الملف')}><X/></button></div>
      ...
      : <><p><strong>{t('Mounir Fatmi · Crate 4', 'منير فاطمي · الصندوق ٤')}</strong> · <bdi>{CASE_ID}</bdi></p>
        ...
        {modal === 'handover' && <><label className="lr-checkbox lr-declaration"><input type="checkbox" checked={acknowledged} onChange={event => setAcknowledged(event.target.checked)}/>{t('I have reviewed this sample report and acknowledge the proposed handover for demonstration purposes. This records no legal custody or digital signature.', 'راجعت هذا التقرير التجريبي وأؤكد التسليم المقترح لأغراض العرض. لا يسجل ذلك حيازة قانونية أو توقيعاً رقمياً.')}</label><p className="lr-small">{t('Recorded as DEMO-DIRECTORATE · declaration DEMO-ACK-1 · browser time. No action is attributed to Mr. Al Qaseer.', 'يُسجل باسم DEMO-DIRECTORATE · الإقرار DEMO-ACK-1 · توقيت المتصفح. لا يُنسب أي إجراء إلى الأستاذ القصير.')}</p><div className="lr-actions"><button onClick={() => setModal(null)}>{t('Cancel', 'إلغاء')}</button><button className="lr-primary" disabled={!acknowledged || actor !== 'DIRECTORATE' || !canReview || reviewVersion !== state.condition?.version} onClick={() => { dispatch({ type: 'ACCEPT', ...envelope(), reportVersion: reviewVersion, acknowledged }); setModal(null); }}>{t('Confirm demo handover', 'تأكيد التسليم التجريبي')}</button></div></>}
      </>}
    </dialog>
  </div>;
}
