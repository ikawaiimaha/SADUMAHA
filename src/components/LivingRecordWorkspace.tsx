...
export function LivingRecordWorkspace() {
  ...
  return <div className="living-record" lang={lang} dir={isAr ? 'rtl' : 'ltr'}>
    ...
    <main className="lr-main">
      <div className="lr-context">{actor === 'DIRECTORATE' ? <span>{t('DEPARTMENT PORTFOLIO · SAMPLE DATA', 'محفظة الأقسام · بيانات تجريبية')}</span> : <><span>{t('FICTIONAL CASE · SESSION ONLY', 'حالة افتراضية · لهذه الجلسة فقط')}</span><bdi>{CASE_ID}</bdi><span>{t('Mounir Fatmi — demonstration scenario', 'منير فاطمي — سيناريو توضيحي')}</span></>}</div>
      ...
      <header className={`lr-anchor ${leadership ? '' : 'lr-anchor--work'}`}>
        ...
        <div><p className="lr-eyebrow">{t('SHARJAH DEPARTMENT OF CULTURE', 'دائرة الثقافة في الشارقة')}</p>
          ...
          <p className="lr-subtitle">{actor === 'CHAIRMAN' ? t('Chairman of the Department of Culture · Executive exception brief', 'رئيس دائرة الثقافة · موجز الاستثناءات التنفيذية') : actor === 'DIRECTORATE' ? t('Director of Cultural Affairs · Department performance and schedules', 'مدير إدارة الشؤون الثقافية · أداء الأقسام والجداول الزمنية') : actor === 'MANAGER' ? t('Assigned delivery management · sample role', 'إدارة التنفيذ المكلفة · دور تجريبي') : t('One record. A clear next action.', 'سجل واحد وخطوة تالية واضحة.')}</p>
      ...
      {actor === 'CHAIRMAN' ? <div className="lr-brief">
        <section className="lr-panel"><p className="lr-eyebrow">{formatNumber(1, { minimumIntegerDigits: 2 })}</p><h2>{t('Active Commitments', 'الالتزامات الجارية')}</h2>
          ...
          <button className="lr-link" onClick={() => go('DIRECTORATE')}>{t('View department oversight', 'عرض متابعة الأقسام')}<ArrowRight/></button>
        </section>
      ...
      </div> : actor === 'DIRECTORATE' ? <DirectorateOversight/> : <>
