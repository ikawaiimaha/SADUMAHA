...
export function LivingRecordWorkspace() {
  ...
  return <div className="living-record" lang={lang} dir={isAr ? 'rtl' : 'ltr'}>
    ...
    <main className="lr-main">
      ...
      {actor === 'SELECTION' ? <ProposalSelection/> : actor === 'ARTIST' ? <ArtistIntake onCoordinator={() => go('COORDINATOR')}/> : actor === 'CHAIRMAN' ? <ChairmanBrief onDirectorate={() => go('DIRECTORATE')} onPublishing={() => go('PUBLISHING_MANAGER')} onFinance={() => setModal('finance')}/> : actor === 'DIRECTORATE' ? <DirectorateOversight/> : actor === 'PUBLISHING_MANAGER' ? <section className="lr-panel"><h2>{t('Studies and Publishing · sample review', 'الدراسات والنشر · مراجعة تجريبية')}</h2><PublishingCase actor={actor}/></section> : <>
