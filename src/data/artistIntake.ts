...
export const INTAKE_PROGRAMMES = [
...
export type ProgrammeId = typeof INTAKE_PROGRAMMES[number]['id'];
...
export function validateIntake(draft: IntakeDraft, programmeId: ProgrammeId): IntakeIssue[] {
  const issues: IntakeIssue[] = []; const p = draft.profile; const q = draft.proposals[programmeId];
  ...
  const amounts = Object.values(q.budget); const budget = INTAKE_PROGRAMMES.find(p => p.id === programmeId)!.budget;
...
export function recoverableDraft(input: IntakeDraft): IntakeDraft {
  ...
  for (const { id } of INTAKE_PROGRAMMES) {
