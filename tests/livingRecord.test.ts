...
const accept = { type: 'ACCEPT', actor: 'DIRECTORATE', reportVersion: 1, acknowledged: true, at } as const;
const actors: DemoActor[] = ['CHAIRMAN', 'DIRECTORATE', 'LOGISTICS', 'TECHNICAL', 'COORDINATOR', 'FINANCE', 'OBSERVER'];
...
test('receipt and evidence do not independently clear the executive readiness gate', () => {
  ...
  assert.equal(state.acceptance?.actor, 'DIRECTORATE');
});
...
test('all out-of-sequence, mismatched, unacknowledged and wrong-role attempts are inert', () => {
  ...
  for (const actor of actors) {
    ...
    if (actor !== 'DIRECTORATE') assert.equal(reduce(ready, { ...accept, actor }), ready);
  }
});
...
test('missing statement changes evidence completeness independently and requires assigned follow-up', () => {
  ...
  state = reduce(state, { type: 'ASSIGN_STATEMENT', actor: 'DIRECTORATE', at });
  state = reduce(state, { type: 'RESTORE_STATEMENT', actor: 'COORDINATOR', at });
  assert.equal(selectLivingRecord(state).evidencePercent, 100);
  assert.equal(state.statementTask, false);
});

test('Finance and Directorate must each act before an executive review item appears', () => {
  let state = createLivingRecord();
  const escalate = { type: 'ESCALATE_FINANCE', actor: 'DIRECTORATE', at } as const;
