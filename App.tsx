...
function SADUApp() {
  ...
  return (
    <div className={`min-h-screen bg-sadu-cream text-sadu-charcoal flex flex-col font-sans selection:bg-sadu-brick selection:text-white ${density === 'compact' ? 'text-xs' : ''}`}>
      ...
      {/* Main Workspace Stage with Strict Role-Based Access Control (RBAC) */}
      ...
      {showBackToTop && (
        <button
          onClick={scrollToTop}
          className="fixed bottom-6 end-6 rtl:end-auto rtl:start-6 z-30 p-2.5 bg-sadu-brick text-white rounded-full shadow-lg hover:bg-sadu-brick-dark transition-all cursor-pointer animate-in fade-in"
      ...
      <footer className="border-t border-sadu-gold bg-sadu-linen mt-12 py-5 px-4 sm:px-6 text-xs text-sadu-muted">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-start rtl:sm:text-end">
          <div>
