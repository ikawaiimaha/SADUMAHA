...
export interface VendorBid {
  ...
  vendorName: string;
  vendorNameAr?: string;
  isRegisteredSupplier: boolean; // Certified with Sharjah Central Finance Department
  submittedPrice: number; // in AED
  technicalApproval: boolean; // Technical evaluation sign-off
...
export interface ArtworkRecord {
  ...
  dimensionsCm: string; // e.g. "180 × 120 × 8 cm"
  weightKg: number;
...
export interface CommitteeReviewSubmission {
  ...
  riskAssessment?: string;
  scores: {
    artisticMerit: number; // /10
    culturalRelevance: number; // /10
    technicalFeasibility?: number; // /10 (Optional/delegated to Museum)
  };
  hasPreviousParticipation?: boolean;
